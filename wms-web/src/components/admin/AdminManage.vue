  ...
  <div>
    ...
    <el-table :data="tableData"  border stripe :header-cell-style="{background:'#f2f5fc',color:'#555555'}">
      ...
      <el-table-column prop="roleId" label="角色" width="120">
        <template v-slot:default="scope">
          <el-tag type="warning" disable-transitions>管理员</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog
      ...
      <el-form ref="form" :rules="rules" :model="form" label-width="80px">
        <el-form-item label="账号" prop="no">
          <el-col :span="20">
            <el-input v-model="form.no" :readonly="read"></el-input>
          </el-col>
        </el-form-item>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      tableData: [],
      //查询的参数
      total: 0, pageNum: 1, pageSize: 10, name: '', sex: '',
      sexes: [{value: '1', label: '男'}, {value: '0', label: '女'}],
      //对话框
      dialogTitle: '',
      centerDialogVisible: false,
      form: {
        id: '', no: '', name: '', password: '', age: '', phone: '', sex: '0', roleId: '1'
      },
      //表单数据检查
      rules: {
        no: [
  ...
  methods: {
    ...
    saveOrUpdate() {
      this.$axios.post(this.$httpUrl + '/wms/user/saveOrUpdate', this.form).then(res => res.data).then(res => {
        if (res.code === 200) {
          this.$message({
            message: '操作成功！',
            type: 'success'
    ...
    del(id) {
      this.$axios.get(this.$httpUrl + '/wms/user/delete?id=' + id).then(res => res.data).then(res => {
        if (res.code === 200) {
          this.$message({
            message: '操作成功！',
            type: 'success'
    ...
    loadPost() {
      this.$axios.post(this.$httpUrl + '/wms/user/listPage', {
