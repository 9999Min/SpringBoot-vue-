...
<script>
export default {
  name: "GoodstypeeManage",
  data() {
    let checkDuplicate = (rule, value, callback) => {
      if (this.form.id && this.nameBeforeEdit === this.form.name) {//编辑模式并且分类名没有改变
        return callback();
      }
      this.$axios.get(this.$httpUrl + "/wms/goodsType/findByName?name=" + this.form.name).then(res => res.data).then(res => {
        if (res.code === 400) callback();
        else callback(new Error('分类名已经存在'));
      })
    };
    return {
      nameBeforeEdit: '',
  ...
  methods: {
    ...
    saveOrUpdate() {
      this.$axios.post(this.$httpUrl + '/wms/goodsType/saveOrUpdate', this.form).then(res => res.data).then(res => {
        if (res.code === 200) {
          this.$message({
            message: '操作成功！',
            type: 'success'
    ...
    del(id) {
      this.$axios.get(this.$httpUrl + '/wms/goodsType/delete?id=' + id).then(res => res.data).then(res => {
        if (res.code === 200) {
          this.$message({
            message: '操作成功！',
            type: 'success'
